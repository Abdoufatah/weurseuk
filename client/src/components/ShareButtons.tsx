...
const NETWORKS = [
  {
    ...
    /**
     * WhatsApp : titre en gras + auteur + accroche + URL
  ...
  },
  {
    ...
    /**
     * X/Twitter : titre + auteur + accroche tronquée (limite 280 chars)
     * L'image og:image est chargée automatiquement par X via le scraping de l'URL.
     * Format : "Titre" — Auteur | Excerpt… + URL
     */
    getUrl: (url: string, title: string, excerpt?: string, authorName?: string) => {
      const byline = authorName ? ` — ${authorName}` : "";
      const base = `"${title}"${byline}`;
      // Réserver ~30 chars pour l'URL raccourcie par X (t.co)
  ...
  },
  {
    ...
    /**
     * Facebook : partage via l'URL OG (/api/og/editorial/:slug)
     * Facebook scrape og:title, og:description, og:image depuis cette URL.
     * Le titre, l'auteur, l'accroche et la photo sont dans les balises OG.
     */
  ...
  },
  {
    ...
    /**
     * LinkedIn : titre + auteur dans le titre, excerpt en résumé
     * LinkedIn scrape og:image depuis l'URL partagée.
     */
    getUrl: (url: string, title: string, excerpt?: string, authorName?: string) => {
  ...
  },
  {
    ...
    /**
     * Telegram : titre + auteur + accroche tronquée + URL
     * Format : Titre\nPar Auteur\n\nExcerpt…
     */
    getUrl: (url: string, title: string, excerpt?: string, authorName?: string) => {
      const byline = authorName ? `\nPar ${authorName}` : "";
...
export default function ShareButtons({
  ...
  const handleShare = async (network: (typeof NETWORKS)[number]) => {
    ...
    // Sur mobile, utiliser navigator.share (menu natif iOS/Android)
    ...
    // Sur desktop : ouvrir le sharer dans une popup
    // Facebook utilise ogUrl pour que le scraper récupère les bonnes métadonnées OG
