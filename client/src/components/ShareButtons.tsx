...
const NETWORKS = [
  {
    ...
    getUrl: (url: string, title: string, excerpt?: string) =>
      `https://wa.me/?text=${encodeURIComponent(excerpt ? `*${title}*\n\n${excerpt}\n\n${url}` : `*${title}*\n\n${url}`)}`,
  },
  {
    name: "X (Twitter)",
    color: "#000000",
    ...
    getUrl: (url: string, title: string, excerpt?: string) => {
      const text = excerpt ? `${title} — ${excerpt.substring(0, 120)}${excerpt.length > 120 ? '…' : ''}` : title;
  ...
  },
  {
    ...
    getUrl: (url: string, _title: string, _excerpt?: string) =>
  ...
  },
  {
    ...
    getUrl: (url: string, title: string, excerpt?: string) => {
      ...
      return `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}${summary}`;
    },
  },
  {
    name: "Telegram",
    ...
    getUrl: (url: string, title: string, excerpt?: string) => {
      const text = excerpt ? `${title}\n\n${excerpt.substring(0, 200)}${excerpt.length > 200 ? '…' : ''}` : title;
...
export default function ShareButtons({
  ...
  const handleShare = async (network: (typeof NETWORKS)[number]) => {
    ...
    const shareLink = network.getUrl(effectiveUrl, title, excerpt);
